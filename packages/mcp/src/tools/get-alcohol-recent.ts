...
export const GetAlcoholRecentInputSchema = z.object({
...
export type GetAlcoholRecentInput = z.infer<typeof GetAlcoholRecentInputSchema>;
...
export function makeGetAlcoholRecentTool(deps: ToolDeps): Tool<GetAlcoholRecentInput> {
  const { api } = deps;
  return {
    name: "get_alcohol_recent",
    description:
      "List alcohol sessions in a date range. Prefer from_date/to_date (YYYY-MM-DD) — the API resolves those to the user's profile timezone. Use from/to (ISO instants) only when you need sub-day precision. Defaults to the last 14 days.",
    inputSchema: GetAlcoholRecentInputSchema,
    annotations: {
      readOnlyHint: true,
      idempotentHint: true,
      openWorldHint: false,
    ...
    handler: async (input) => {
      ...
      const alcohol_sessions = await api.request<unknown[]>(
        "GET",
        `/api/v1/alcohol-sessions?${params.toString()}`,
        undefined,
        { bearer: deps.currentToken() },
      );
      return { alcohol_sessions, count: alcohol_sessions.length };
    },
  };
}
