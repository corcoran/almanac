...
them (like HTTP requests), and receive structured responses, except the
"client" is an LLM, not a browser. The server's job is to make the operations
...
resources, so the client now knows it can ask for a tool list and read
resources.
...
characters, since it costs context on every session. The client injects these
into the LLM's system prompt. The instructions cover:

- Timestamp conventions: naked local strings are interpreted in the user's
  profile timezone, and only `Z` means the user explicitly gave UTC.
...
- Refusal boundaries: don't invent kcal, don't edit history without
  ...
  advice.
- Onboarding: if the user profile is incomplete or there's no active nutrition
  phase, walk the user through setup. `get_next_best_action` reports the next
  unmet setup step, and once set up, forgotten daily logs, so the client
  doesn't have to infer state.

**3. Tool catalog.** The client calls `tools/list` and receives the full tool
set. Each entry includes:

- The name: `log_meal`, `get_weight_trend`, `delete_workout`.
- The description: LLM-facing prose explaining when and how to use the tool,
...
- The input schema: a JSON Schema object generated from the tool's Zod
  definition. Field-level descriptions are embedded, so `eaten_at` explains the
...
(`get_next_best_action`), so a fresh client has one orienting call to start
from. This gives the AI a map of the system on first use, and a unit test
fails if a registered tool isn't represented in the catalog.

Onboarding discoverability is deliberately layered: the catalog
(`recommended_entrypoint`) points to a dedicated `get_next_best_action` tool
that reports current setup state, which is backed by per-tool follow-up hints.
That layering is what lets an LLM peer reliably find "what's next?" rather than
inferring it from `phase: null`.
...
`almanac://exercises`. Clients can read these for ambient context without
making tool calls, which is useful for pre-loading state into an AI's context
window at session start. Each resource is a thin wrapper over a single API GET
request.
...
1. **Zod validation.** `LogMealInputSchema.parse(input)` checks types, ensures
   kcal is a nonnegative integer, and so on, failing immediately with field
   paths if anything is wrong.
...
3. **Idempotency key.** A SHA-256 hash of the canonical JSON payload, prefixed
   with `meal:<userId>:` and sent as the `idempotency-key` header.
...
5. **Context fetch.** `GET /api/v1/signals/macros?at=2026-05-25T12:30:00` gets
   the day's running totals and target, so the response can include "500 kcal
   logged; day total now 1200/2400 (50%)" without a second tool call from the
   AI.
...
**Phase coherence.** `update_phase` runs a `checkPhaseInvariant()` before
calling the API: a "cut" phase must have a deficit below -5% of TDEE, a "bulk"
must be above +5%, and "maintenance" must be within ±5%. Ambiguous patches that
would violate these constraints are rejected immediately with a structured
error explaining the conflict.

**Workout shape constraints.** `log_workout` uses `.refine()` to enforce that
the caller provides either `exercises` (ad-hoc) or `deviations` from a
template, not both. At least one of `exercises` or `template_id` must be
present.
...
`server_now` is the wall-clock time in the user's profile timezone at the
moment the response was generated, and `user_tz` is the IANA timezone name.
This anchors the AI to real time without it having to ask "what time is it?"
or compute timezone offsets. The timezone is resolved lazily from the user
profile on the first tool call and cached for the session, then invalidated
when `update_user_profile` changes it, so the next response stamps the new
zone.
...
The key design choice: structured API errors, whether validation failures or
business rule violations, are passed through to the AI as readable JSON, not
generic "something went wrong" messages. This lets the AI diagnose and retry
intelligently.

## Response shaping

Tool handlers don't just proxy API responses. They often make follow-up reads
to give the AI useful context in a single round-trip:
...
This is intentional: AI tool calls are expensive, since each one is a
round-trip through the LLM, so responses are designed to be self-contained
enough that the AI rarely needs to chain multiple reads.
...
- The stdio transport uses a static PAT from the `ALMANAC_MCP_CLIENT_TOKEN`
  environment variable.
- The HTTP transport extracts it from the `Authorization: Bearer` header on
  the initial request. When OAuth 2.1 is enabled, the token is minted during
  the OAuth flow: provider sign-in, email allowlist check, PAT generation.
...
The only thing that matters across restarts is the PAT itself, and that's in
the database. Clients that lose their registration re-register and
re-authorize; the OAuth spec handles this gracefully.
...
schemas. This keeps the two layers in sync on enum values and domain
constraints without coupling their full request/response shapes: the MCP
tools define their own input schemas optimized for AI ergonomics, such as
optional fields with descriptive defaults, while the API schemas enforce the
