...
because the images must exist before the server can pull them.
...
Second, the workflow finished successfully. A failed build leaves no new image,
and watchtower sits there with nothing to pull:

```bash
gh run list --workflow=release.yml --limit 5
```

Until that run is green, nothing on the server changes. If you find yourself
debugging a "stale" server, check this first.
:::
...
git SHA baked into the image at build time, so if it still shows the old value
the swap hasn't happened yet (or has failed). Also confirm `migrations_applied`
moved if the release carried a migration.
...
and respecting healthchecks, exactly as `update.sh` does manually. **After the
...
- Watchtower runs with `--label-enable`, so it only touches containers carrying
  `com.centurylinklabs.watchtower.enable=true`, which is just the three almanac
  services. oauth2-proxy is deliberately excluded, because its image is pinned
  and must never auto-move.
- Public images (the default) need no credentials. Watchtower mounts the
  deploying user's `~/.docker/config.json` read-only, so private images reuse
  whatever `docker login` wrote.
- Watchtower can mail you on update or error via shoutrrr SMTP. Set
  `WATCHTOWER_EMAIL_TO` in `$ALMANAC_DIR/.env` to turn that on; leave it blank
  and watchtower runs silently, so you'd notice a stale app only by checking
  ...
  FQDN, since shoutrrr's default HELO is the literal `localhost`, which a
  strict postfix rejects with `504 5.5.2 ... need fully-qualified hostname`.
- `--cleanup` removes the old image after each successful swap, so pulled
...
Silence in that log for well over five minutes means it isn't polling, so check
...
notifications entirely. Watchtower then fails silently, and a broken pull looks
...
Unlike `update.sh`, watchtower doesn't snapshot the DB before swapping. That's
an accepted tradeoff: the nightly `scripts/backup-db.sh` cron (see
[Backups](#backups)) is the safety net. If a release carries a risky migration
and you want a fresh snapshot first, deploy that one manually with
`./deploy/update.sh` (which backs up first), or run `scripts/backup-db.sh` by
hand before pushing the tag.

A second, narrower safety net costs you nothing: the API itself snapshots the
database at startup whenever there are **pending migrations**, writing
`data/backups/pre-<tag>-<timestamp>.sqlite` and logging `pre-migration snapshot
written` with the path. A plain restart with no schema change writes nothing.
That covers the watchtower path, but it's a last resort rather than a substitute
for scheduled backups, because it only ever captures the moment immediately
before a migration.

Watchtower keys off the *tag* a container was started with, normally `:latest`.
Pin a rollback (`ALMANAC_TAG=0.2.0`, see below) and those containers run
`:0.2.0`, which rarely moves, so a pin won't get stomped by a new `:latest`. It
also won't auto-resume `:latest` updates until you clear `ALMANAC_TAG` and
...
- `ALMANAC_TAG=0.2.0 ./deploy/update.sh` pulls and runs a specific published tag
  instead of `latest`, and it's the rollback lever described below. The effect
  is transient: it applies only to this invocation, and the next bare
  `./deploy/update.sh` reverts to `:latest`. To pin durably, set
  `ALMANAC_TAG=0.2.0` in `$ALMANAC_DIR/.env`.
- `DRY_RUN=1 ./deploy/update.sh` pulls, prints the `up -d --dry-run` recreate
  plan, and stops without mutating anything.
- `SKIP_BACKUP=1` skips the DB snapshot, which is a bad idea before a migration.
- `ALMANAC_DB_PATH=...` overrides the SQLite path when your data dir isn't the
...
`almanac-api`. Read both, since the tail is where a failing migration surfaces.
...
If `update.sh` fails on its first line, that's the preflight: it runs `docker
compose version` and exits 1 when Compose isn't reachable. It also runs
...
The backup step fails for two reasons: no database file at `ALMANAC_DB_PATH`, or
no `sqlite3` CLI on the host. `backup-db.sh` exits non-zero and names which one.
Fix the cause rather than reaching for `SKIP_BACKUP=1`; skipping the snapshot
before a migration is exactly the situation the snapshot exists for.
...
That pin is transient, and the next bare `./deploy/update.sh` goes back to
...
in which case `docker compose pull` would have errored. Scroll back through the
`update.sh` output for the pull failure rather than assuming the swap silently
declined.
:::
...
forward. That's usually harmless, but if the bad release's migration is what
broke you, a code rollback alone won't fix it. Restore the snapshot as well, per
the next block.
:::
...
`pre-<tag>-<UTC-stamp>.sqlite`. Newest first:
...
Old pulled images accumulate, so reclaim space periodically, though **never
...
Pruning removes the older image layers you would roll back *to*. Prune when
you're confident in the current release, not while you're standing next to a
fresh one.

## Backups

Wire `scripts/backup-db.sh` into a cron job. The script honors `ALMANAC_DB_PATH`,
so the server's data directory works without edits. Use absolute paths, because
cron doesn't see your shell's `$ALMANAC_DIR`:
...
Cron email needs a working MTA. The `MAILTO` line only delivers if the box has
postfix/sendmail/msmtp installed and relaying somewhere you read, and minimal
...
nonexistent file) before trusting it. If the box has no MTA, drop `MAILTO`, add
`2>&1` to the cron line so stderr lands in the log rather than going nowhere,
and monitor the logfile (or an external dead-man's-switch like healthchecks.io)
instead.

Backups land in `<deploy-dir>/data/backups/`. Once you have a weekly rhythm,
it's worth moving them off-server (S3, Backblaze, etc.).

### How the script behaves

Worth knowing before you rely on it:

- It uses `sqlite3 .backup`, the online backup API, so it's safe to run while
  the API holds the database open. It also forces a WAL checkpoint into the
  snapshot, so the output is a single self-contained file and you don't need to
  copy `-wal` / `-shm` alongside it.
- Output is `data/backups/almanac-<UTC-stamp>.sqlite`, where the stamp is
  `YYYYMMDDTHHMMSSZ`.
- It prunes automatically, keeping the newest 30 by default. Override that with
  `ALMANAC_BACKUP_KEEP`.
...
Run it by hand first, since it's a read-only operation against the live
database:
...
snapshot is actually usable, because a backup you've never opened is a
...
- `docker` and `jq` on the host, or it aborts with `missing tool on host`
- the compose stack is running; it aborts if `almanac-api` isn't in
  `docker compose ps --status running`
- `TEST_EMAIL` is set, or it exits immediately
...
| 7 | The revoked PAT is rejected with 401 | Revocation isn't taking effect, which is a serious auth finding. |
...
live, so check Settings → Tokens and revoke it by hand.
:::
...
Copy the existing SQLite file into the server's `data/` directory (the host side
of compose's `./data:/data` mount) **before first boot**:
...
behind, and the main file's mtime won't warn you, because it doesn't change on
every write.
...
If `users` is empty or the file doesn't parse, do not boot: you'll be creating a
fresh instance, and the first-admin bootstrap will fire against an empty table
...
logs the path, which is your rollback point if the import goes sideways.
:::

File ownership matters here. The container writes to `/data` through the
`./data:/data` bind mount, and an imported file copied in as `root` may not be
writable by the container's user. SQLite also needs to create `-wal` and `-shm`
siblings in that directory. If the API logs a read-only or permission error on
boot, check ownership on both the file and the directory.

## Next steps

- [Deploy](/guide/deploy) is the one-time stand-up
- [Authentication](/guide/authentication) covers OAuth setup and auth failure
  modes
- [Configuration](/guide/configuration) lists every environment variable
