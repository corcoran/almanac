...
::: tip Compose already pins three of these
...
| `ALMANAC_MCP_CLIENT_TOKEN` | Static PAT the MCP process uses for its own API calls. Consumed **only** under `stdio`, which has no incoming request to read a bearer from, so this is the process's identity. Boot fails if it's missing under stdio. | stdio |
...
| `OAUTH2_PROXY_SKIP_PROVIDER_BUTTON` | Skip oauth2-proxy's "Sign in with X" interstitial and go straight to the provider. Defaults to `true`, since Almanac only ever configures one provider and the page is just an extra click. Set `false` to restore it. | optional |
...
::: warning Set all four, or none of them
...
::: warning `WATCHTOWER_EMAIL_HELO` needs a real FQDN
...
Two things keep that small. The cheaper model does the high-volume work, with
meal parsing on Haiku and only the coach reaching for Sonnet, and the system
...
::: warning The chat buttons stay hidden until you do this
...
the second, the per-user flag gives you the first, and setting only the env
vars looks like nothing happened.
:::
...
- [Getting started](/guide/getting-started): run it locally, then connect your assistant
